...
For a longer treatment with worked examples, see the type theory tutorial. The proposed substrate realisation collapses this to one propagation engine, with the structural/user-domain distinction expressed as where a property's transfer rules come from (the proven core versus user declarations) rather than two engines or a stored tag, in [lineage-facts.md](lineage-facts.md); if that design is adopted this section gets reconciled to that framing.
...
### Cross-model contracts inherit a backing

A claim that spans more than one model earns the name "contract" only when the framework can back it the way it backs everything else. The headline example is aggregate conservation, a rollup that should tie out to its source. Conservation is not a new primitive. It is a theorem whose side conditions are properties the substrate already propagates: the grand-total identity `sum(rollup.x) == sum(source.x)` holds when the group-by partition is total and survives downstream (the **nullability** property on the group key, since a nullable key quarantines null rows into a phantom group a later join drops), when no row is double-counted (the **uniqueness and grain** property behind the fan-out finding), when the filters on each side agree (the **predicate engine** that conditional-fact activation already runs), and when the summed values share a coherent tag (the **domain-type** property). Conservation is provable when all four discharge and a hazard is provable when one is violated, so the motivating "revenue by zip stops tying out" bug is a nullability-through-aggregation question the property already raises with no contract authored at all.

Generalizing from that example gives a clean admissibility test. Every cross-model claim is verified one of three ways, and dblect records which:

- **A propagating property.** The strongest claims are themselves lattice values the substrate carries along lineage: a domain type, nullability, uniqueness, grain, a functional dependency, referential integrity. These are cross-model for free because propagation is cross-model, and they inherit the framework's rigor wholesale (lattice and semiring laws under property tests, an empirical soundness oracle against a warehouse). Adding propagating properties is the highest-leverage way to grow cross-model coverage, which is why the functional dependency carries so much weight: `country -> currency` declared once discharges the currency-coherence obligation on an aggregation many models away, and being checkable, a row that violates it becomes its own finding. The functional dependency is uniqueness generalized from "this set determines everything" (a candidate key is `K -> *`) to "this set determines that set," so it lifts the carrier the substrate already moves rather than adding a new propagation.
- **An algebraic certificate (a sound static fast-path).** When both sides of a relation between aggregates reduce, through non-opaque operators, to relational-algebra terms over a common ancestor, the equality is provable without running anything, read through provenance semirings as the two sides carrying equal annotations in the counting semiring (the same `N`-semiring the fan-out check uses). This is the aggregate-query-equivalence problem (Cohen, Nutt, and Sagiv; Chaudhuri and Vardi for bag semantics), decidable on the select/project/join/union/group-by fragment and unavailable the moment a side passes a window region, a recursive CTE, or a UDF, the type-erasing boundaries the substrate already marks.
- **A property-based oracle (the default for everything else).** A claim relating two quantities that meet only at the comparison (`sum(A) == sum(B)`, a ratio, an inequality) has no single lineage path to carry it, so the generator manufactures its failure shapes, runs the models, and shrinks any failure to a minimal counterexample. A pass is "not refuted by the shapes we tried," the trust class the framework attaches to anything it checks rather than proves.

A claim that is none of these is an assertion with no verification mode, and the framework should not dress it as a contract; the `@contract.check` escape hatch is exactly that weakest class, kept visible and rare. The same discipline means a passing check is worth different things in different regimes, and the report should say which: derivation-provable (the partition theorem above, exact, refuted the instant a side condition breaks), type-and-grain-checkable (independent relations whose domain types and grains must still line up, so "these two totals are not the same kind of thing" is caught with no data), or runtime-confirmed (the tie-out itself under data, within a tolerance that records the genuine rounding-and-timing budget rather than muting a real drift). The authoring surface follows the theory rather than leading it: a claim lives where its fact holds and propagates from there, and the spelling stays deferred until each side condition is pinned to a property with its soundness obligation at the aggregation site.
