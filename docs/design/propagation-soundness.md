...
For some properties the confluence combine turns out to be exactly the lattice join, and then the two structures coincide. This happens when merging branches and comparing precision are the same operation, which is the case for an *agreement* property: branches that agree keep the value, branches that disagree fall to the top "no information", and that is what the lattice join already does. A value-domain property like currency is the clean case: two branches on the same currency keep it, two on different currencies merge to the top "unknown currency", exactly the join.

Idempotence alone does not buy this, and nullability is the instructive counterexample. It is idempotent, a `UNION ALL` of two nullable branches is nullable, yet its confluence is *not* the lattice join. The precision lattice has to keep `unknown` at the top, since `unknown` is "no information": the value an opaque upstream takes, the identity an empty resolution folds to, and the value the consistency check must never fail on. But a confluence has to let a proven `nullable` branch dominate an `unknown` one, because the nullable arm's nulls reach the output whatever the other arm holds. A join with the top is always the top, so no lattice with `unknown` on top can give `join(nullable, unknown) = nullable`. Nullability is an *existential* property (null may arrive from any branch), and an existential confluence lets a committed value win over the top, which the join cannot.

A counting property breaks the coincidence the other way, and seeing why is the clearest way to understand the split. Take a property tracking how many rows each output stands for. At a `UNION ALL` of a two-row branch and a three-row branch the output has five rows, so the confluence combine is addition. Addition is not the join of any lattice on counts, because a lattice join is idempotent (`x` joined with `x` is `x`) while addition is not (`2 + 2` is `4`, not `2`). The combine therefore lives outside the precision lattice and has to be supplied separately.

That separate structure is a **semiring**: a `plus` for confluences (the `UNION ALL` combine, addition for counting, the null-taint for nullability) and a `times` for crosses (the `JOIN` combine, multiplication for counting, since joining an m-row relation with an n-row relation yields up to m times n rows), each with an identity element. A `Property` carries an optional `semiring` slot, filled whenever the confluence orders values differently from the precision lattice (the counting and accumulating properties, and existential properties like nullability whose committed values must beat the top), and left empty for the agreement properties (value-domain meanings such as currency or tax inclusion, and uniqueness) whose confluence is exactly the lattice join.
...
- **Confluence (`UNION ALL`).** The property's confluence combine. For an agreement property this is the lattice join (uniqueness: a key survives only if both branches carry it, so an unknown branch drops it), forced by the lattice. An existential or counting property supplies the semiring `+` instead (nullability: nullable if either branch is, so a proven nullable arm taints even an unknown one; counting: addition). `UNION DISTINCT` is the same confluence with one operator-specific addition for uniqueness: set semantics removes duplicate rows, so the whole projected row is a *superkey* (a column set that is unique, though perhaps not minimal). The rule is therefore keyed on the specific operator.
...
- **Confluence only, no join combine.** A value-domain, agreement, or existential column property has no cross combine to define at a join, because each output column is a copy of exactly one input column (a join projects columns through; it does not blend two columns' annotations into one). With no cross combine, a join has nothing to distribute over, and plan-independence reduces to the confluence combine being associative and commutative, which it is, whether that combine is the lattice join or the property's semiring `plus`.
...
- **Semiring laws** when a `semiring` is present (associativity, commutativity, distributivity, and the identity and annihilation roles). This is what buys plan-independence for the properties whose confluence is not the lattice join. The `plus` need not equal the lattice join: nullability's does not, since its confluence lets a committed value beat the "no information" top while the join cannot.
