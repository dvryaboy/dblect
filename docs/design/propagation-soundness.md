...
- **Confluence only, no join combine.** A value-domain or idempotent column property has no cross combine to worry about at a join, because each output column traces to exactly one input column (a join column on the left is the same value join-side or not). So there is nothing for a join to distribute over, and plan-independence reduces to the confluence combine being associative and commutative, which it is, since it is the lattice join.
