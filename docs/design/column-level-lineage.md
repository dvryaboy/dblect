...
- **Nullability.** `K = {non-null, nullable, unknown}`, a 3-element precision lattice for resolution, with a separate idempotent semiring for the combine: `+` and `×` both take the more-null value, so a `UNION ALL` or expression is nullable as soon as one input is. The combine is not the lattice join, since `unknown` is the lattice top yet a proven `nullable` has to dominate it. Sources seed from declared `not_null` tests.
