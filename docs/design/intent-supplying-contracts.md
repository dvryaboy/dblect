...
**What the analysis decides, and what it does not.** It decides the shape of the violation, not the fix. It proves the conserved measure fans out and is not renormalized, or that it is dropped, and it names the hop. It can confirm one structural fix, a divisor equal to the fan-out degree (`count(*) over (partition by order_id)`), because that provably cancels the replication the fan-out fact measured. It cannot certify a data-dependent apportionment: that `ml.attribute`'s slices sum to the order's revenue is a property of the data the UDF returns, discharged by running or by a second declared conservation contract, not by static analysis.
...
**How the declaration closes the gap.** `grain(per=...)` lowers to a `CandidateKeySet` fact, and `uniqueness` already computes the model's inferred candidate keys. The declared grain and the inferred keys disagree: the author says `order_id` determines a row, the analysis finds the model keyed per line. That disagreement is the finding, raised at the model where the grain is wrong rather than at the downstream sum that eventually trips over it. The fix is to aggregate to the declared grain, or to correct the declaration to the grain the SQL produces.

**What we do with it.** The facts flow, but the emitter does not exist yet. Domain-type contradictions surface as findings today (`DOMAIN_TYPE_CONTRADICTION`, `JOIN_KEY_TYPE_MISMATCH`, both raised by the check when a declared type meets a conflicting inferred one); the key and grain analogue does not. Connecting it is an emitter that compares the declared grain against the inferred candidate keys and reports the conflict, the uniqueness twin of the domain-type contradiction check.
...
**How the declaration closes the gap.** `determines` lowers to an `FDSet` fact carried on the `functional_dependency` property, and it opens two consumers. It sharpens the existing detectors at once, the same way an inferred dependency does, so a fan-out that `zip -> city` proves harmless is suppressed. And it becomes checkable against structure: a `UNION` whose branches carry conflicting `zip -> city` bindings, or a select that leans on the dependency where the structure does not support it, contradicts the declared fact. The fix is a single source of truth for the mapping before the union, or dropping the claim if the data does not support it.

**What we do with it.** The sharpening is live: a declared `determines` reaches the grounding detectors today. The self-check is the missing emitter, the functional-dependency twin of section 4's grain-contradiction reader: compare the declared dependency against what the structure supports and report the conflict.
...
The wiring falls into a few kinds. Two shapes want a reader that walks a declared relation across the lineage (conservation, the referential orphan drop). Two want an emitter that surfaces a declared-versus-inferred contradiction the substrate already computes but does not report for keys and dependencies (grain, FD). One wants a facet in the type algebra (non-additive), one a small check over an accepted set (dead predicate), and one a new value-interval property (range). The common thread is the premise: the declaration supplies the intent that turns an ambiguous structural signal into a decision, and where the trace cannot derive the fact, the declaration stands in for it.
