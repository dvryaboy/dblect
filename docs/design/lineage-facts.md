...
The framework's licence to *infer* a transfer (rather than ask the user) follows the four composition rules the digest names, and they are exactly the operator and aggregate transfers a property registers:

- **preserve**: an identity transfer (`Alias`, a bare `Column`). The value passes through unchanged. This is where tightening happens.
- **erase**: a transfer that returns the property default (an opaque scalar function, a bare literal touching a `VOUCHED` axis). The value drops to `UNKNOWN`. This is where tightening stops, and where a `VOUCHED` property asks for an annotation rather than guessing.
...
- **Tightening and boundary.** For a `PROVEN` property, an inferred value strictly more precise than the declaration propagates the inferred value as the flow value, keeps the declared value as the boundary, and emits the soft "can tighten" finding. For a `VOUCHED` property, the same through a `preserve` chain, and an `erase` step stops the tightening.
