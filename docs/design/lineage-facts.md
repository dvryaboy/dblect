...
Combinability says whether a measure's meaning survives the property's combination under a `GROUP BY` or aggregate, and under what precondition. The combination is the property's semiring operation (arithmetic sum for a numeric measure, set union for candidate keys, a max or boolean monoid for others), so the honest word is "combines," not "adds." A measure is `Combinable(by=<monoid>)` when that monoid preserves its meaning over any grouping, `Combinable(by=..., within=<cols>)` when it survives only where those columns are constant in the scope (the coherence precondition that becomes a `depends_on` edge on the functional-dependency property), `SemiCombinable(over=<dims>)` when it survives over some dimensions and not others, and `NotCombinable` when no aggregate preserves it. In the design prose this is the measure being *monoidal* under the aggregate; on the surface the user writes `Combinable`.
...
The user never writes any of this. The `depends_on` edge for the currency example originates in a type-layer declaration like `Combinable(by=SUM, within="currency")`, which says this measure combines under sum only where the currency is constant. The framework compiles that intent into the dependency and the transfer; the user-land vocabulary lives in the types layer.
