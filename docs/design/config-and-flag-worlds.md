...
Enforcement is read off the *materialization*, not the incremental strategy alone, and the candidate-key fact carries that enforcement standing (the analog of `NativeConstraint.enforced_on_write`). Incremental `merge`-with-key and `delete+insert` enforce, and so do **snapshots**, which enforce uniqueness on their key without being incremental, so they are a first-class enforcing case (snapshot support lands in [#52](https://github.com/dvryaboy/dblect/issues/52)). Reading enforcement from the materialization keeps the enforcing set extensible as new enforcing materializations are recognized, rather than tied to an incremental-strategy check that would miss snapshots. The single-batch hazard rides as its own detector rather than as a caveat on the key fact, so the enforced key stays unconditional and the hazard stays a legible separate finding.
...
Several questions this design opened are now settled or tracked; the rest stay genuinely open.

1. **Config-derived keys: enforced or asserted? (resolved.)** Enforcement is read off the materialization and the candidate-key fact carries that standing; snapshots are a first-class enforcing case ([#52](https://github.com/dvryaboy/dblect/issues/52)); the single-batch hazard is its own detector. See "Worked example" above.
2. **Default responsiveness granularity. (decided for v1.)** Model-responsiveness grounds the flag only where a model reads its var, which degrades to silence rather than a wrong fact. Lineage-directed (column-level) responsiveness is the precision refinement, tracked with cone scoping in [#99](https://github.com/dvryaboy/dblect/issues/99).
3. **Per-world SQL for control-flow worlds. (resolved in direction.)** The enumerator is fact-level (one shared graph, facts vary per world); manifest-per-world recompilation is rejected as the general mechanism at monster scale. Control-flow worlds, whose SQL differs, are evaluated by model-scoped deduplicated recompilation patched into the shared graph, tracked in [#100](https://github.com/dvryaboy/dblect/issues/100).
4. **When to start the variability-aware front end. (gated, open trigger.)** The empirical fork's five seams make the trigger measurable from compiled SQL and lineage before the front end is built; the front end is the path for general `{% if var %}` control-flow worlds in [#100](https://github.com/dvryaboy/dblect/issues/100). The threshold itself stays open pending measurement on real projects.
5. **Interaction clusters under structure-changing flags. (open, tracked.)** When a flag changes the DAG (`enabled`, a join behind `{% if %}`, a gated `ref`), the influence sets are world-dependent; the conservative union over induced structures is sound but can over-cluster. Whether a tighter sound bound is worth the complexity is open, scoped with cone-based per-contract narrowing in [#99](https://github.com/dvryaboy/dblect/issues/99).
6. **Flag discovery and classification. (tracked.)** Discovering vars, classifying each as control-flow / value-substitution / computed, and surfacing only the control-flow subset as world axes is var-inference, tracked in [#98](https://github.com/dvryaboy/dblect/issues/98).
