...
A contract is checked at a node (a column or relation with a declared value). That node's value is a function of the flags whose responsive code lies upstream of it, and of no others. The where-provenance property already computes, per node, the set of source columns whose values feed it; intersecting that influence cone with the responsive scopes of each flag yields the node's **flag-influence set**: the only flags that can change whether this contract holds. A flag absent from the set cannot move the node's value, so it cannot break the contract, so its values never need to be varied while checking it.
