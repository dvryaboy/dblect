...
- [**`builder.py`**](../../src/dblect/lineage/builder.py): turns compiled SQL into the graph. Per model it calls sqlglot's lineage walker and stamps each `Column` in the projection expression with the real source columns it resolves to. CTE intermediates collapse here: a CTE column built from `a.x + a.y` stamps the outer reference with both leaves. The cross-model variant walks the manifest DAG and merges per-model graphs; per-model failures land as `BuildIssue` entries instead of blanking the whole build. When a caller shares its already-parsed tree (the audit does), the builder also writes each resolved `ColumnRef` back onto that tree's nodes, across every reference position rather than just projections, so a detector can read the builder's resolution (`resolved_column_ref`) instead of re-deriving lexical scope. Qualification still runs on a private copy, so the shared tree's structure and line numbers are untouched; only `.meta` is enriched. The inner-flatten detector consumes this to clear an `UNNEST` of an array proven non-empty through a CTE or model boundary.
...
1. Pre-parses every model's `compiled_code` once and hands the trees to `make_fact_grounded_detectors(manifest, dialect=..., parsed=...)`, which propagates the uniqueness property over the relation graph once and curries the fact-grounded detectors against the resulting per-relation keys (reusing the same `Expr` per model rather than re-parsing). The manifest column graph is likewise built once over those shared trees (the heavy qualify-and-resolve walk that also stamps them with their resolved refs) and handed to every column-graph fact family, so the walk runs a single time per audit rather than once per family. The curried detectors join the configured `detectors` list so the per-model loop runs everything in one pass.
