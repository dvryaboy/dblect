...
**A finding carries two spans.** `Finding.line_start`/`line_end` are the compiled-SQL span the parser saw (the raw observation). The walker then back-maps that span onto the on-disk `.sql` template via `audit/sourcemap.py`, and `LocatedFinding.located_span` reports the result: a `SourceSpan` whose `basis` is `SOURCE` when the construct aligns to a source line and `COMPILED` when it originates inside an expanded macro with no clean source line. The back-map aligns compiled and `raw_code` line-for-line on verbatim-passthrough content (whitespace collapsed, so a re-indent still matches) and anchors only where it is sure: a line rewritten by compilation (a `ref()` becoming a relation name) does not match and stays compiled-relative rather than being mapped to a guessed line. The text report marks a compiled-relative span `(compiled)`; the JSON carries `source_line_start`/`source_line_end`/`line_basis` alongside the compiled `line_start`/`line_end`; SARIF places the region on the source span and omits it when the span could not be back-mapped, so a code-scanning UI never highlights a guessed line. Findings always carry the model's `original_file_path` either way.

`-- noqa` suppression matches on the same `located_span`. Directives live in `raw_code` and are read in source-line space, so matching them against the back-mapped source span is what lets a directive on the line the report shows silence a finding whose compiled line a macro expansion shifted. A construct emitted inside a macro has no source line, so its span stays compiled-relative and suppression falls back to the compiled line; suppressing those reliably is open work (#162): a source line the developer can annotate does not exist for macro-emitted SQL, so it needs a mechanism beyond line back-mapping.
