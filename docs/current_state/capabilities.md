...
4. **Suppression for declaration findings** (#117). The real-project shakedown that calibrates false-positive rates beyond the jaffle and scenario fixtures is done and written up in [calibration.md](./calibration.md): four DuckDB projects, 88 models, zero logic false positives, with the determination that `join_fanout` is the one detector whose `error` default should wait on surrogate-key grounding or the lenient/strict split (#116).
