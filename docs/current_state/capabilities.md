...
4. **Suppression for declaration findings** (#117). The real-project shakedown that calibrates false-positive rates beyond the jaffle and scenario fixtures is done and written up on [#125](https://github.com/dvryaboy/dblect/issues/125): six projects across DuckDB, Snowflake, and Spark, 172 models. The precision follow-ups it opened have since landed, the flatten and `WHERE` idiom fixes (#210), the per-site finding collapse (#211), and the Spark struct-field lineage fix (#212), which cut Wikimedia's findings from 37 to 20, cleared its 11 unbuilt models, and left one `null_group` guarded-grouping gap as the remaining precision follow-up. The pass demotes `join_fanout`/`cross_model_fanout` to advisory pending surrogate-key grounding or the lenient/strict split (#116).
