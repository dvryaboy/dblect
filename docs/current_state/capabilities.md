...
4. **Suppression for declaration findings** (#117). The real-project shakedown that calibrates false-positive rates beyond the jaffle and scenario fixtures is done and written up in [calibration.md](./calibration.md): six projects across DuckDB, Snowflake, and Spark, 172 models, 47 findings hand-classified (9 true positives, 23 sound-but-intended, 15 false positives). The false positives concentrate in three fixable precision gaps plus per-column finding multiplicity, and the pass demotes `join_fanout`/`cross_model_fanout` to advisory pending surrogate-key grounding or the lenient/strict split (#116).
