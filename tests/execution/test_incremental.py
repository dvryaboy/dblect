...
These tests require ``dbt-core`` / ``dbt-duckdb`` importable and the ``dbt`` CLI
on PATH, the same gating as the rest of the execution harness; they skip
otherwise. The mechanism is pinned at the boundary: a project directory in, two
per-world ``Manifest``s out, with the ``is_incremental()`` branch present in the
steady-state world and absent in the full-refresh world.
"""
...
def test_both_worlds_compile(incremental_project_dir: Path) -> None:
    from dblect.execution.incremental import compile_incremental_worlds

    result = compile_incremental_worlds(incremental_project_dir)
    assert result.full_refresh.ok, result.full_refresh.error
    assert result.steady_state.ok, result.steady_state.error


def test_worlds_carry_the_incremental_assignment(incremental_project_dir: Path) -> None:
...
def test_watermark_branch_present_only_in_steady_state(incremental_project_dir: Path) -> None:
    from dblect.execution.incremental import compile_incremental_worlds
...
def test_structure_adding_branch_only_in_steady_state(incremental_project_dir: Path) -> None:
    from dblect.execution.incremental import compile_incremental_worlds
