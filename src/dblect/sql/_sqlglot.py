...
def literal_constant(e: Expr) -> exp.Literal | exp.Boolean | None:
    ...
    ``exp.Boolean``, not a ``Literal`` subclass, but they are constants just the same.
    ``NULL`` is not a literal here: it is the absence of a value, not a constant one,
    so it never unwraps to one."""
